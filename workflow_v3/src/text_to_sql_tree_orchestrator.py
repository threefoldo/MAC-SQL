...
class TextToSQLTreeOrchestrator:
    ...
    async def _get_tree_results(self) -> Dict[str, Any]:
        ...
        for node_id, node_data in tree["nodes"].items():
            ...
            # Extract execution result from generation field (primary) or evaluation field (fallback)
            execution_result = None
            if node_data.get("generation") and isinstance(node_data["generation"], dict):
                execution_result = node_data["generation"].get("execution_result")
            if not execution_result and node_data.get("evaluation") and isinstance(node_data["evaluation"], dict):
                execution_result = node_data["evaluation"].get("execution_result")
    ...
    async def display_query_tree(self) -> None:
        ...
        for node_id, node_data in tree["nodes"].items():
            ...
            # Show execution result from generation field (primary) or evaluation field (fallback)
            exec_result = None
            if node_data.get('generation') and isinstance(node_data['generation'], dict):
                exec_result = node_data['generation'].get('execution_result', {})
            if not exec_result and node_data.get('evaluation') and isinstance(node_data['evaluation'], dict):
                exec_result = node_data['evaluation'].get('execution_result', {})
            ...
            # Show evaluation quality if evaluation exists
    ...
    async def display_final_results(self) -> None:
        ...
        if final_sql:
            ...
            if root_id and root_id in tree.get("nodes", {}):
                ...
                # Show execution result from generation field (primary) or evaluation field (fallback)
                exec_result = None
                if root_node_data.get('generation') and isinstance(root_node_data['generation'], dict):
                    exec_result = root_node_data['generation'].get('execution_result', {})
                if not exec_result and root_node_data.get('evaluation') and isinstance(root_node_data['evaluation'], dict):
                    exec_result = root_node_data['evaluation'].get('execution_result', {})
                ...
                if exec_result:
                    ...
                    # Show quality assessment if evaluation exists
