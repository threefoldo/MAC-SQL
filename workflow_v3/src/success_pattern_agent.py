    ...
    async def _reader_callback(self, memory: KeyValueMemory, task: str, cancellation_token) -> Dict[str, Any]:
        ...
        try:
            ...
            context = {
                ...
                # Get execution result from generation field (primary) or evaluation field (fallback)
                "execution_result": json.dumps(
                    node.generation.get("execution_result", {}) if node.generation and "execution_result" in node.generation
                    else node.evaluation.get("execution_result", {}) if node.evaluation
                    else {}, 
                    indent=2
                ),
