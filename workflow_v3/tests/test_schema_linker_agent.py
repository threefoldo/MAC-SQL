...
class TestSchemaLinkerAgent:
    ...
    async def setup_test_environment(self, query: str, task_id: str, node_intent: str, db_name: str = "california_schools", evidence: str = None):
        ...
        # Schema linking data will be stored directly in nodes
    ...
    async def test_agent_only_prepares_schema_and_extracts_output(self):
        ...
        # Create schema linking agent with proper LLM config
        agent = SchemaLinkerAgent(memory)
        agent.llm_config = {
            "model_name": "gpt-4o",
            "temperature": 0.1,
            "timeout": 60
    ...
    async def test_schema_formatting_only(self):
        ...
        agent = SchemaLinkerAgent(memory)
        agent.llm_config = {"model_name": "gpt-4o"}
    ...
    async def test_trust_llm_output_without_validation(self):
        ...
        # Handle both single table and list formats
        if isinstance(tables, list):
            table_item = tables[0]
        else:
            table_item = tables
        assert table_item["name"] == "non_existent_table"
