...
class TestSQLEvaluatorAgent:
    ...
    async def test_agent_only_formats_results_for_llm(self):
        ...
        # Evaluation data will be stored directly in the node
        from datetime import datetime
        
        # Create evaluator (workflow_v3 pattern)
        agent = SQLEvaluatorAgent(memory)
        agent.llm_config = {
            "model_name": "gpt-4o",
            "temperature": 0.1,
            "timeout": 60
        ...
        # Get evaluation from node (not global memory)
        tree_manager = QueryTreeManager(memory)
    ...
    async def test_no_validation_against_patterns(self):
        ...
        for i, test_case in enumerate(test_cases):
            ...
            # Create and run evaluator (workflow_v3 pattern)
            agent = SQLEvaluatorAgent(memory)
            agent.llm_config = {
                "model_name": "gpt-4o",
                "temperature": 0.1,
