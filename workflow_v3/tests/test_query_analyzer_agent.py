...
class TestQueryAnalyzerAgent:
    ...
    async def _initialize_schema_linking(self, memory: KeyValueMemory, query: str, db_name: str, with_schema_analysis: bool = False):
        """NO global memory operations - agents only save data in nodes"""
        # Schema linking data will be stored directly in nodes by SchemaLinkerAgent
        # This method is kept for test compatibility but doesn't set global memory
        from datetime import datetime
        
        # Optionally add pre-analyzed schema for testing (future: store in nodes)
    ...
    async def test_agent_only_prepares_context_and_extracts_output(self):
        ...
        # Create analyzer with proper LLM config
        ...
        print(f"Complexity: {analysis.get('complexity', 'N/A')} (LLM decided, not agent)")
        ...
        # Verify XML parsing worked (check for either format)
        last_message = result.messages[-1].content
        assert ("<analysis>" in last_message or "<query_analysis>" in last_message)
    ...
    async def test_context_preparation_only(self):
        ...
        analyzer = QueryAnalyzerAgent(memory)
        analyzer.llm_config = {"model_name": "gpt-4o"}
