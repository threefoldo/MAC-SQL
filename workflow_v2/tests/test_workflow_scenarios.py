...
class WorkflowTestScenarios:
    ...
    async def setup_bird_database(self):
        """Load BIRD database schema for testing."""
        # Load BIRD dev_tables.json
        with open('/home/norman/work/text-to-sql/MAC-SQL/data/bird/dev_tables.json', 'r') as f:
            dev_tables = json.load(f)
        
        # Use financial database which has good complexity
        db_info = dev_tables['financial']
        ...
        for table_name, columns_desc in db_info['desc_dict'].items():
            ...
            for i, col_name in enumerate(columns_desc):
                col = ColumnInfo(
                    name=col_name,
                    data_type=db_info['type_dict'][table_name][i],
                    is_primary_key=(col_name in pk_info),
                    is_foreign_key=(col_name in [fk[0] for fk in fk_info]),
                    nullable=True,
                    description=f"Column {col_name} in {table_name}",
                    sample_values=columns_val.get(col_name, [])[:5] if col_name in columns_val else []
                )
                columns.append(col)
            ...
            table = TableSchema(
                ...
                description=f"Table {table_name} in financial database"
            )
            tables.append(table)
    ...
    async def scenario_1_simple_aggregation(self):
        ...
        # Setup database
        await self.setup_bird_database()
    ...
    async def scenario_2_complex_joins(self):
        ...
        task = TaskContext(
            ...
            database_name="financial",
            selected_tables=["client", "account", "trans", "disp"],
            current_sql="",
            execution_result={},
            is_complete=False
        )
    ...
    async def run_all_scenarios(self):
        ...
        # Export memory state
        memory_state = await self.memory.export_to_json()
